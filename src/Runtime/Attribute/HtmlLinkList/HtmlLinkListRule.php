...
class HtmlLinkListRule extends Rule
{
    // href URL options
    ...
    protected function execute($values, EntityInterface $entity = null)
    {
        ...
        $link_rule = new HtmlLinkRule('html-link', $this->getOptions());

        foreach ($values as $index => $val) {
            if (!$link_rule->apply($val)) {
                $this->throwIncidentsAsErrors($link_rule, null, [ 'path_parts' => [ $index ] ]);
                return false;
            }

            $sanitized[] = $link_rule->getSanitizedValue();
        }
